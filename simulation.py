"""Core simulation helpers for the ACAS/TCAS Monte Carlo Streamlit app."""
...
def vs_time_series(
    ...
    """Vertical speed law with delay, ramp, and optional saturation."""
    a = a_g * G
...
def integrate_altitude_from_vs(
    times_s: np.ndarray, vs_fpm: np.ndarray, z0_ft: float = 0.0
) -> np.ndarray:
...
def compliance_score_method_b_like(
    ...
    """Proxy for Method‑B compliance scoring."""
...
def sample_initial_vs_with_aggressiveness(
    ...
    """Sample an initial vertical speed consistent with aggressiveness."""
    if leveloff_context:
        if rng.uniform() < 0.90:
...
def simulate_miss_for_senses(
    ...
    times, vs_pl = vs_time_series(
        tgo, dt, pl_delay, pl_accel, pl_vs, sense=sense_pl, cap_fpm=pl_cap, vs0_fpm=pl_vs0
    )
    _, vs_ca = vs_time_series(
        tgo, dt, cat_delay, cat_accel, cat_vs, sense=sense_cat, cap_fpm=cat_cap, vs0_fpm=cat_vs0
    )
...
def run_batch(
    ...
    for k in range(int(runs)):
        ...
        comp_label = compliance_score_method_b_like(
            sense_required=sense_cat_exec, vs_trace=vs_ca, times=times
        )
