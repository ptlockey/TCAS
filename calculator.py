...
-----------------------------------------------------------------------
Implements the edits we discussed:
...
C) Constrain RA timing to ~15–35 s prior to CPA (typical TCAS trigger window).
D) Re‑tune non‑compliance priors toward EUROCONTROL radar‑based compliance envelopes.
E) Level‑off contexts when aggressiveness=0.0 (fully left): near‑level VS sampling
   to emulate the frequency of “Level off, level off” RAs in congested airspace.
F) Optional AP/FD TCAS effect (shorter CAT delay & slightly higher accel).
G) More conservative reversal trigger (rare by design).
H) Lower TA‑only prevalence among RA participants (~0.2–0.5% baseline).
I) Reaction‑time mixture: CAT ≈70% ~4.5 s / 30% ~8 s; PL delay ≈2.2–2.5 s.
J) Method‑B‑like compliance label (Following/Weak/No/Opposite/Excessive) & richer logging.
USAGE
-----
...
Flags:
  ...
  --aggr                 Initial trajectory aggressiveness (0.0 = level‑off context)
  ...
  --no-delay-mixture     Disable CAT delay mixture (use single delay model)
  --hdg1-min/--hdg1-max/--hdg2-min/--hdg2-max   (for Custom)
  --out                  Output CSV path
...
# Performance-limited (PL) aircraft defaults (more conservative)
PL_DELAY_MEAN_S = 2.2    # s (manual/limited performance feel)
PL_DELAY_SD_S   = 0.4
PL_ACCEL_G      = 0.10
PL_VS_FPM       = 500.0  # fpm
PL_VS_CAP_FPM   = 500.0  # fpm
# CAT (transport) nominal corrective behaviors (v7.1 assumptions)
CAT_INIT_VS_FPM      = 1500.0    # initial corrective RA target
CAT_STRENGTH_FPM     = 2500.0    # strengthened target (Increase Climb/Descend)
...
def ias_to_tas(ias_kt: float, pressure_alt_ft: float) -> float:
    """Rough TAS from IAS using ISA density ratio sigma."""
...
def sample_headings(
    ...
    hdg2_min: Optional[float] = None, hdg2_max: Optional[float] = None
) -> Tuple[float, float]:
...
def alim_ft_from_alt(pressure_alt_ft: float) -> float:
    """Simplified ALIM model: ~300–700 ft across 10k–35k ft."""
...
def compliance_score_method_b_like(
    ...
    """Simplified proxy for Method B scoring.
...
def sample_initial_vs_with_aggressiveness(
    rng: np.random.Generator, aggressiveness: float, leveloff_context: bool = False
) -> float:
...
def simulate_miss_for_senses(
    tgo: float, dt: float, h0: float, cat_above: bool,
    sense_pl: int, sense_cat: int,
    pl_delay: float, pl_accel: float, pl_vs: float, pl_cap: float, pl_vs0: float,
    cat_delay: float, cat_accel: float, cat_vs: float, cat_cap: float, cat_vs0: float
) -> float:
...
def choose_optimal_sense(
    tgo: float, dt: float, h0: float, cat_above: bool, pl_vs0: float, cat_vs0: float,
    cat_delay_nom: float = 5.0, cat_accel_nom: float = 0.25,
    cat_vs: float = CAT_INIT_VS_FPM, cat_cap: float = CAT_CAP_INIT_FPM
) -> Tuple[Tuple[int, int], float, float]:
    # PL up / CAT down
    miss_updown = simulate_miss_for_senses(
        tgo, dt, h0, cat_above,
        sense_pl=+1, sense_cat=-1,
        pl_delay=PL_DELAY_MEAN_S, pl_accel=PL_ACCEL_G, pl_vs=PL_VS_FPM, pl_cap=PL_VS_CAP_FPM, pl_vs0=pl_vs0,
        cat_delay=cat_delay_nom, cat_accel=cat_accel_nom, cat_vs=cat_vs, cat_cap=cat_cap, cat_vs0=cat_vs0
    )
    # PL down / CAT up
...
def apply_non_compliance_to_cat(
    rng: np.random.Generator, sense_cat: int,
...
def classify_event(
    times: np.ndarray, z_pl: np.ndarray, z_ca: np.ndarray,
    vs_pl: np.ndarray, vs_ca: np.ndarray, tgo: float,
    alim_ft: float, margin_ft: float,
    sense_chosen_cat: int, sense_exec_cat: int
...
def apply_second_phase(
    times: np.ndarray, vs_pl: np.ndarray, vs_ca: np.ndarray,
    ...
    decision_latency_s = float(np.clip(decision_latency_s, 0.6, 1.4))
    t2_issue = float(times[-1] - max(0.0, tgo - times[-1])) + decision_latency_s
    ...
    t2, vs_pl2 = vs_time_series(t_rem, dt, pl_delay, pl_accel_g, pl_cap,
                                sense=new_sense_pl, cap_fpm=pl_cap, vs0_fpm=vs_pl_now)
    _,  vs_ca2 = vs_time_series(t_rem, dt, cat_delay, cat_accel_g, cat_vs_strength,
...
def sample_altitudes_and_h0(
    ...
    h0_lo: float = 100.0, h0_hi: float = 500.0
) -> Tuple[int, int, float]:
...
def run_batch(
    runs: int = 5000, seed: int = 26, scenario: str = "Head-on",
    ...
    dt: float = 0.1,
    hdg1_min: float = 0.0, hdg1_max: float = 360.0, hdg2_min: float = 0.0, hdg2_max: float = 360.0
) -> pd.DataFrame:
    ...
    for k in range(int(runs)):
        ...
        # RA timing: clamp to 15–35 s (never below 15 s)
        if scenario == "Head-on":
            mu, sd = 25.0, 5.0
        elif scenario == "Crossing":
            mu, sd = 22.0, 6.0
            ...
            mu, sd = 30.0, 8.0
        lo = TGO_MIN_S
        hi = min(tgo_geom if tgo_geom is not None else TGO_MAX_S, TGO_MAX_S)
...
def parse_args(argv=None):
    ...
    p.add_argument('--no-delay-mixture', action='store_true', help='Disable CAT delay mixture (use single delay)')
...
def main(argv=None):
    ...
    df = run_batch(
        ...
        use_delay_mixture=(not args.no_delay_mixture),
