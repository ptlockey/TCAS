# app_streamlit.py
# ACAS/TCAS Monte‑Carlo v2 (Streamlit) — optimal-sense + non‑compliance + 2‑phase RA
# Restores Streamlit UI and embeds the v7.1‑aligned logic.
import math
...
def compliance_score_method_b_like(sense_required: int, vs_trace: np.ndarray, times: np.ndarray,
    ...
    """Very simplified proxy for IATA/EUROCONTROL Method B."""
...
def sample_initial_vs_with_aggressiveness(rng: np.random.Generator, aggressiveness: float,
    ...
    if leveloff_context:
        # “Level‑off” context: mostly near‑level VS, occasional small ±250 fpm
        if rng.uniform() < 0.90:
...
def apply_second_phase(times: np.ndarray, vs_pl: np.ndarray, vs_ca: np.ndarray,
                       ...
                       t2_issue_override: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, Optional[float]]:
    """Second‑phase execution. If t2_issue_override provided, use it for reproducible plots."""
...
# ------------------------- Batch engine (returns DataFrame) ------------------------- #
@dataclass
class BatchCfg:
    runs: int = 5000
    seed: int = 26
    scenario: str = "Head-on"    # Head-on | Crossing | Overtaking | Custom
    r0_min_nm: float = 4.0
    r0_max_nm: float = 8.0
...
def run_batch(cfg: BatchCfg) -> pd.DataFrame:
    rng = np.random.default_rng(int(cfg.seed))
    if cfg.scenario == "Head-on":
        rel_min, rel_max = 150.0, 210.0
    elif cfg.scenario == "Crossing":
        rel_min, rel_max = 60.0, 120.0
    elif cfg.scenario == "Overtaking":
        rel_min, rel_max = 0.0, 30.0
    else:
        rel_min = rel_max = None
    data: List[Dict] = []
    for k in range(int(cfg.runs)):
        ...
        if cfg.scenario == "Custom":
            h1 = float(rng.uniform(cfg.hdg1_min, cfg.hdg1_max))
            h2 = float(rng.uniform(cfg.hdg2_min, cfg.hdg2_max))
        else:
            h1, h2 = sample_headings(rng, cfg.scenario, 0.0, 360.0, rel_min, rel_max)
        r0 = float(rng.uniform(min(cfg.r0_min_nm, cfg.r0_max_nm), max(cfg.r0_min_nm, cfg.r0_max_nm)))
        vcl = relative_closure_kt(PL_TAS, h1, CAT_TAS, h2)
        tgo_geom = time_to_go_from_geometry(r0, vcl)
        # RA timing 15–35 s
        if cfg.scenario == "Head-on": mu, sd = 25.0, 5.0
        elif cfg.scenario == "Crossing": mu, sd = 22.0, 6.0
        else: mu, sd = 30.0, 8.0
        ...
        leveloff_context = (cfg.aggressiveness <= 1e-6)
        vz0_pl  = sample_initial_vs_with_aggressiveness(rng, cfg.aggressiveness, leveloff_context)
        vz0_cat = sample_initial_vs_with_aggressiveness(rng, cfg.aggressiveness, leveloff_context)
        # Optimal sense (nominal CAT)
        (sense_pl, sense_ca), miss_nominal, miss_alt = choose_optimal_sense(
            tgo, cfg.dt, h0, cat_above, vz0_pl, vz0_cat,
            cat_delay_nom=5.0, cat_accel_nom=0.25, cat_vs=CAT_INIT_VS_FPM, cat_cap=CAT_CAP_INIT_FPM)
        # CAT delay mixture + AP/FD
        if cfg.use_delay_mixture:
            if rng.uniform() < 0.7:
            ...
            cat_delay_eff = 5.0; cat_accel_eff = 0.25
        if rng.uniform() < cfg.apfd_share:
        ...
        mode, sense_cat_exec, cat_delay_exec, cat_accel_exec, cat_vs_exec, cat_cap_exec = apply_non_compliance_to_cat(
            ...
            p_opp=cfg.p_opp, p_taonly=cfg.p_ta, p_weak=cfg.p_weak, jitter=cfg.jitter_priors)
        # PL delay
        pl_delay = max(0.0, rng.normal(PL_DELAY_MEAN_S, PL_DELAY_SD_S))
        # First phase time series
        times, vs_pl = vs_time_series(tgo, cfg.dt, pl_delay, PL_ACCEL_G, PL_VS_FPM, sense=sense_pl,
                                      cap_fpm=PL_VS_CAP_FPM, vs0_fpm=vz0_pl)
        _, vs_ca = vs_time_series(tgo, cfg.dt, cat_delay_exec, cat_accel_exec, cat_vs_exec, sense=sense_cat_exec,
        ...
        # Second phase (store issue time for later plot reconstruction)
        t2_issue = None
        if eventtype in ("STRENGTHEN", "REVERSE"):
            # Use a deterministic 1.0 s latency here; the exact value is not critical for batch metrics
            t2_issue = float(times[-1] - max(0.0, tgo - times[-1])) + 1.0
            times2, vs_pl2, vs_ca2 = apply_second_phase(times, vs_pl, vs_ca, tgo, cfg.dt, eventtype,
                                                        sense_pl, sense_cat_exec,
        ...
        data.append(dict(
            run=k+1, scenario=cfg.scenario,
            FL_PL=FL_PL, FL_CAT=FL_CAT,
            PL_TAS=PL_TAS, CAT_TAS=CAT_TAS,
            PLhdg=h1, CAThdg=h2,
            R0NM=r0, closurekt=vcl, tgos=tgo, h0ft=h0,
            aggressiveness=cfg.aggressiveness, leveloff=int(leveloff_context),
...
# --- Single‑run testbed (visual check) --- #
...
with spot_c3:
    spot_dt = st.number_input("Plot dt (s)", 0.01, 1.0, dt, 0.01)
if st.button("Run single simulation (optimal sense)"):
    rng = np.random.default_rng(1234)
    # Simple fixed geometry for single‑run (headings randomized)
    PL_IAS = 120.0
    FL_PL, FL_CAT, _ = 250, 260, 0.0
    PL_TAS = ias_to_tas(PL_IAS, FL_PL*100.0)
    CAT_TAS = float(rng.uniform(420.0, 470.0))
    ...
    st.pyplot(fig)
    st.caption(f"Chosen sense: PL = {'climb' if sense_pl>0 else 'descend'}, CAT = {'climb' if sense_ca>0 else 'descend'}")
st.markdown("---")
# --- Batch Monte‑Carlo --- #
...
cfg = BatchCfg(
    ...
    jitter_priors=bool(jitter_priors),
    apfd_share=float(apfd_share),
    use_delay_mixture=bool(use_delay_mixture),
    dt=float(dt)
)
...
# --- Results, metrics, plots, download --- #
if "df" in st.session_state:
    ...
    p_opp    = (df['comp_label'] == "Opposite").mean()*100
    ...
    plot_dt = st.number_input("Plot dt (s)", 0.01, 1.0, dt, 0.01)
    ...
    # Reconstruct time series from saved parameters (including second‑phase t_second_issue)
